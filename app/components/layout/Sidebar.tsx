...
export default function Sidebar({
  ...
  return (
    <aside
      className={`fixed top-0 left-0 h-screen bg-[var(--background-secondary)] border-r border-[var(--border-color)] transition-all duration-300 z-[90] ${isOpen ? 'w-64' : 'w-[72px]'
        } overflow-hidden`}
    >
      <div className="flex flex-col h-full pt-16">
        ...
        >
          ...
          <span
            className={`text-sm font-medium text-[var(--text-primary)] whitespace-nowrap transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0'
              }`}
          >
            New Chat
          </span>
        ...
        {chats.length > 0 && (
          <div className="px-4 py-3 border-b border-[var(--border-color)]">
            <span
              className={`text-xs font-medium text-[var(--text-secondary)] transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0'
                }`}
            >
              Recent
            </span>
        ...
        <div className="border-t border-[var(--border-color)]">
          <button className="flex items-center gap-3 px-4 py-3 w-full hover:bg-[var(--hover-background)] transition-colors">
            <svg className="w-6 h-6 text-[var(--text-secondary)] flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <span
              className={`text-sm text-[var(--text-secondary)] transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0'
                }`}
            >
              Help
            </span>
